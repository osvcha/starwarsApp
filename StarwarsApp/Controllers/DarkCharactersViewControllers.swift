...
class DarkCharactersViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        tabBarController?.title = "Dark Side Characters"
    }
    ...
    private func loadData() {
        characters = characterRepository.characterSample.filter({ character in
            return character.side.sideName == .dark
        })
    }
    
}

extension DarkCharactersViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if(indexPath.row < characters.count){
            performSegue(withIdentifier: "SEGUE_FROM_DARKCHARACTER_TO_DETAIL", sender: characters[indexPath.row])
